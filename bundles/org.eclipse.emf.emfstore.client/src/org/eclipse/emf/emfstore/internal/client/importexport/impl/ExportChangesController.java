...
public class ExportChangesController extends ProjectSpaceBasedExportController {
	 ...
	 * 
	 * @param projectSpace
	 *            the {@link ProjectSpace} of which its local changes should be
	 *            exported
	 */
	...
	public void execute(File file, IProgressMonitor progressMonitor)
		...
		if (!FileUtil.getExtension(file).equals(
			ExportImportDataUnits.Change.getExtension())) {
			file = new File(file.getAbsoluteFile()
				+ ExportImportDataUnits.Change.getExtension());
		}

		final AbstractChangePackage changePackage = getProjectSpace()
			.getLocalChangePackage(false);
		ResourceHelper.putElementIntoNewResourceWithProject(file
			.getAbsolutePath(), changePackage, getProjectSpace()
			.getProject());
	}
