...
public class DiffGenerator {
	...
	public void createDiff(TestRun firstRun, TestRun secondRun)
		...
		final Map<String, TestDiff> existingDiffs = new HashMap<String, TestDiff>();
		final List<TestDiff> diffs = diffReport.getDiffs();
		for (final TestDiff diff : diffs) {
	...
	private void checkForDiffs(List<TestResult> firstResults,
		...
		final EList<TestDiff> diffs = diffReport.getDiffs();
		for (final TestResult result : new ArrayList<TestResult>(firstResults)) {
			final TestResult corrResult = getCorrespondingTestResult(result,
				secondResults);

			final TestDiff diff = getChangedTestDiff(result, corrResult);
			if (diff != null) {
	...
	private static TestDiff createTestDiff(TestResult fRes, TestResult sRes) {
		final TestDiff diff = ConfigFactory.eINSTANCE.createTestDiff();
	...
	private static TestResult getCorrespondingTestResult(TestResult result,
		...
		for (final TestResult res : results) {
